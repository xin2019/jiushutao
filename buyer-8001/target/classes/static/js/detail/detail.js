function showImg(index) {
    ...
    $('#showPhoto').attr("src",url_prefix+"/"+index+".jpg");
    $('#maxImg').attr("src",url_prefix+"/"+index+".jpg");
    $('#'+index).attr("style","border:3px solid #eeeeee")
}
...
$('#purChase').click(function () {
    ...
    // if(loginUserPhone==null||bookid==null){
    //     alert("loginUserPhone==null||bookid==null")
    //     $("#myModal").modal('show');
    //     return
    // }else{
    //
    //     console.log("/buyer/getUsername 进入");
    //
    //     //交互后台判断登陆状态
    ...
    //             // alert(sessionStorage['loginUser']);
    //             console.log("进入：/buyer/getBuyerLogin?phone="+sessionStorage['loginUser']);
    //             $.ajax({
    ...
    //                     console.log("获取到的 /buyer/getBuyerLogin?phone= 的data");
    //                     console.log(obj);
    //                     // alert(obj.status);
    //                     if(obj.status=="1"){
...
//点击加入购物车，http://localhost:9001/buyer/cart/addToCart?bookid=1&buyerid=1&quantity=10
    ...
    $.ajax({
        ...
        success:function (data) {
            if(data==true){
                ...
                alert("添加失败，请重试--远程调用9001addtocart服务失败")
            }
        },
        error:function (data) {
            alert("ajax-error--------addCart")
        }
    })
})
...
$('#githubPanel').addClass("hidden");
    ...
    var pic = document.getElementById("showPhoto"),
        mark = document.getElementById("mark"),
        boxRight = document.getElementById("boxRight"),
        img = boxRight.getElementsByTagName('img')[0],
    ...
    pic.addEventListener('mousemove', function (ev1) {
        ...
        img.style.left = -tx * 1.7 + 'px';
        img.style.top = -ty * 2.9 + 'px';
...
// console.log(obj.id);
    ...
    //     // var tt="/buyer/loginout?phone="+sessionStorage['loginUser'];
    //     // alert(tt);
    //     // $.ajax({
    //     //     url: "/buyer/loginout?phone="+sessionStorage['loginUser'],
    //     //     type: "GET",
    //     //
    //     //     datatype: "json",
    //     //     success: function (data) {
    ...
    //                 $("#myModal").modal("show");
    //             } else {
    //                 window.location.href = "cart/accessCart";
    //             }
    //         },
    //         error: function (data) {
    //             alert("请联系管理员" +
...
//
//     for(var i=0;i<cookieArray.length;i++){
//         if(cookieArray.search("userPhone")!=-1){
//             userPhone=cookieArray[i].split("=")[1];
//         }
//     }
//     // alert("test=>"+id);
...
//     var userID=cookie.split("=")[2];
//     if(userPhone!=null&&userPhone!=""&&userPhone!=undefined){
//         window.location.href="/buyer/allOrder?id="+userID;
//     }else{
//         $("#myModal").modal('show');
//     }
// }
...
$('#btnSubmitPersonalInfo').click(function () {
    ...
    $.ajax({
        ...
        error:function (data) {
            console.log("ajax--btnSubmitPersonalInfo------gg")
        }
    })
})
